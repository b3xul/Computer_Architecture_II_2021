...
**
**------------------------------------------------------------------------------------------------------
** Modified by:			Francesco Sattolo
** Modified date:		17/01/2021
** Version:					2.0
** Descriptions:		added custom functions
********************************************************************************************************/
...
void MyDrawLine(uint16_t x0, uint16_t y0, uint16_t size, uint16_t color, LINE_DIRECTIONS dir){
	uint16_t x,y;
	
	switch(dir){
		case HORIZONTAL:
			for(x=x0, y=y0; x<x0+size; x++){
				LCD_SetPoint(x,y,color);
			}
		break;
		case VERTICAL:
			for(x=x0, y=y0; y<y0+size; y++){
				LCD_SetPoint(x,y,color);
			}
		break;
		default:
...
void DrawSquare(uint16_t Xpos, uint16_t Ypos, uint16_t Side, uint16_t Color){
	uint16_t y;
	
	for(y=Ypos; y<Ypos+Side; y++){
		MyDrawLine(Xpos, y, Side, Color, HORIZONTAL);
	}
	return;
}

void DrawRectangle(uint16_t Xpos, uint16_t Ypos, uint16_t sideX, uint16_t sideY, uint16_t Color){
	uint16_t y;
	for(y=Ypos; y<Ypos+sideY; y++){
		MyDrawLine(Xpos, y, sideX, Color, HORIZONTAL);
	}
	return;
}

void DeleteText(uint16_t Xpos, uint16_t Ypos, uint16_t strlen, uint16_t bkColor){
	/* length of the text box = 8 (found in PutChar function) */
	/* height of the text box = 16 (found in PutChar function) */
	DrawRectangle(Xpos, Ypos, strlen*8, 16, bkColor);
	return;
}
